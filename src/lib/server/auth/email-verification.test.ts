...
beforeEach(() => {
	...
	// Reset rate limiter
	sendVerificationEmailBucket.reset();
});
...
describe('integration tests', () => {
	it('should handle complete email verification lifecycle', async () => {
		...
		expect(consoleSpy).toHaveBeenCalledWith(`To ${newEmail}: Your verification code is ${request.code}`);
		consoleSpy.mockRestore();
	...
	it('should handle cookie-based request retrieval', async () => {
		...
		const foundFromInvalidCookie = getUserEmailVerificationRequestFromRequest(mockEventWithInvalidCookie);
