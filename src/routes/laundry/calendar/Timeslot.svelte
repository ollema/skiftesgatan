<script lang="ts">
	...
	import { release, reserve, pb, timeslotBlocked } from '$lib/pocketbase';
	...
	$: blocked = timeslotBlocked(start.toString() + 'Z');
	$: disabled =
		(reservation !== undefined && !reservedByApartment) || (blocked && !reservedByApartment);
