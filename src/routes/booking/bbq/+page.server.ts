...
export const load = (event) => {
	...
	const bookings = getBookings('bbq', startDate, endDate);

	const userBookings = getFutureBookingsPerUser(event.locals.user.id, 'bbq');
	if (userBookings && userBookings.length > 1) {
		console.warn(
			`found more than one future booking for user ${event.locals.user.id} in bbq bookings`
		);
	}
