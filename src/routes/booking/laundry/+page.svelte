...
<div class="container mx-auto p-6">
	<h1 class="mb-6 text-3xl font-bold">Boka tvättstuga</h1>
				...
				{#each data.bookings as booking (booking.id)}
					<div class="flex items-center justify-between rounded-lg border p-4">
						...
						{#if booking.userId === data.user.id}
							<form method="POST" action={route('cancel /booking/laundry')} use:enhance>
	...
	<div class="rounded-lg bg-gray-50 p-6">
		...
		<form method="POST" action={route('create /booking/laundry')} use:enhance class="space-y-4">
			<div>
			...
			<div>
				<label for="slot" class="mb-1 block text-sm font-medium text-gray-700">Tid</label>
				<select
					id="slot"
					name="slot"
					required
				...
				>
					...
					{#each data.laundrySlots as slot, index (index)}
			...
			<button
				type="submit"
				class="rounded-md bg-blue-500 px-6 py-2 font-medium text-white hover:bg-blue-600"
			>
				Skapa bokning
			</button>
		</form>
